...
int fs_rename(const char* src_path, const char* dst_path)
{
    ...
    // get src entry file, dir, or symink inode
    ...
    // get dest entry file, dir, or symink inode
    ...
    if (t_dirno >= 0) {
        if (fs.inodes[src_inum].mode != fs.inodes[dst_inum].mode) {
            return -EINVAL; // report error if  source and dest not same type
        }
        // if target is a directory
        if (S_ISDIR(fs.inodes[dst_inum].mode)) {
            if (!is_dir_empty(dst_inum)) {
                return -EINVAL; // target directory is not empty
            } else {if ((errno = fs_rmdir(dst_path)) < 0) {
                return errno;// report error if cannot remove dest dir or unlink dest file
             }
            }
        }
        // unlink the target
        else{
            if((errno = do_unlink(dst_inum, dstdir_inum, dst_leaf))<0){
                return errno;// report error if cannot remove dest dir or unlink dest file
            };
        }
    }

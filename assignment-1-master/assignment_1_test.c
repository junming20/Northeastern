...
void test_math(void) {
	...
	word w17;
	word w18;
	div2Word(w17, w18, w7, w9);  // -3 / 2 = -1 r -1
	CU_ASSERT_WORD_EQUAL(w17, expected14);
	CU_ASSERT_WORD_EQUAL(w18, expected14);

	word w19;
	word w20;
	div2Word(w19, w20, w6, w7);  // 4 / -3 = -1 r 1
	word expected20 = {0x00, 0x00, 0x00, 0x01};  // 1
	CU_ASSERT_WORD_EQUAL(w19, expected14);
	CU_ASSERT_WORD_EQUAL(w20, expected20);
}
...
void test_logical(void) {
	...
	CU_ASSERT_WORD_EQUAL(mask1, w1);

	word mask2;
	maskWord(mask2, w10, wordsize);
	CU_ASSERT_WORD_EQUAL(mask2, w10);

	word mask3;
	maskWord(mask3, w10, wordsize/4);
	CU_ASSERT_WORD_EQUAL(mask3, w5);

	word mask4;
	maskWord(mask4, w10, 0);
	CU_ASSERT_WORD_EQUAL(mask4, w0);

	word mask5;
	maskWord(mask5, w10, -wordsize/2);
	CU_ASSERT_WORD_EQUAL(mask5, w3);
	...
	word mask7;
	maskWord(mask7, w10, -wordsize/4);
	CU_ASSERT_WORD_EQUAL(mask7, w4);
}
